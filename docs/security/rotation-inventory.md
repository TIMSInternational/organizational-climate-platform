...
|---|---|---|---|
| Supabase Postgres password | Secrets Manager (`DatabaseConnectionStringSecretArn`) → env `ConnectionStrings__ClimateProject` | Supabase project `organizational-climate-platform`, `us-east-1` (README). Runtime uses the **Transaction pooler, port 6543**; EF migrations need the **direct** connection, port 5432 — rotate and verify **both** paths or migrations break later, in a different session, looking unrelated. | ☐ |
