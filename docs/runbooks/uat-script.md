...
| **And then** | `DashboardPage` dispatches on the role claim and, for one role, on the tenant switcher (`DashboardPage.tsx:51-72`): `company_admin` → CompanyAdmin view (`:62-65`), `leader` **and** `supervisor` → `next/team/TeamDashboardPage`, which draws a different view per role — the leader's Panel de Control and the supervisor's, labelled as a proposal (the canvas's LeaderDashboard and SupervisorDashboard, 10 Sep) — both reading the one team endpoint, `employee` **and any unrecognised role** → the Employee view (`:71`). For `super_admin` read the note below before recording anything. Four endpoints, one per view: `GET /dashboard/super-admin`, `/company-admin`, `/department-admin`, `/employee` (`DashboardEndpoints.cs:97-100`) |
...
| 5c | `/surveys/templates/:id` | Read the **template's own name and description** in Spanish and then in English | **They follow the locale when the template was named in both languages** — `survey_templates.name_en`/`name_es` and `description_en`/`description_es` are paired columns since #210, resolved by `AuthoredContent.Resolve` like the questions. A template named in **one** language keeps that name in both locales **and says so**: the detail's `fallbackFields` then lists `name`/`description` beside any question paths, and the page shows the same *Idioma de este contenido* / *Language of this content* notice it shows for questions (`SurveyTemplateDetailPage.tsx`). Which case a seeded template is depends on how it was authored: a bare string is filed under the company's language, or the author's display language when the company is set to `both` | A name that changes with the locale **without** both halves having been authored — that would be the content-mangling #195 forbids. A monolingual template whose page shows **no** language notice when read in the other locale |
...
| 3 | `/dashboard` | Read the team view | The leader's Panel de Control renders: their **own** department, resolved from their user row and never from the token, because people move teams (`DashboardEndpoints.cs:293-299`). Naming somebody else's `departmentId` is a **403, not a lookup** (`:306-310`). Read the note above this table before recording a 200 as a leak. The **org.** bars and the "frente a la org." moves beside the team's scores are **sample data** and say so with a *Datos de muestra* chip on every card that draws them (the legend then names the organisation without a respondent count): no endpoint gives a leader the whole company's reading yet (`web/src/features/dashboard/next/team/sampleModel.ts`). That is not another department's figures and not a defect | Another department's figures; or a 403 on their own team |
...
| 3 | `/dashboard` | Read the team view | The supervisor's Panel de Control — **not** the leader's. Its eyebrow starts *Propuesta* and an amber note says the ruling on the role is pending. Three blocks: coverage of the open survey, the tracking plans they execute, their tasks. Scoped to their own department, read from the same team endpoint | The employee view instead — means the role claim did not arrive; or the leader's view, with its climate comparison |
| 4 | `/dashboard` | *Cobertura de la encuesta abierta*, below and above the floor | Below 5 answers, both *Ya respondieron* and *Faltan por responder* are hatched and print no number — the second because the team's size minus it gives the first back. At 5 or more, both print. There is no climate grid on this panel: it stays on the leader's | Any count under 5, in either box |
...
| 5 | Invitee | `/accept-invitation/:token` (`router.tsx:219`) | Set name and a policy-satisfying password (see §6.6), submit | Account created and signed in. `POST /invitations/{token}/accept` (`InvitationAcceptEndpoints.cs:25`), unauthenticated and rate-limited **per token** (`:22-26`) | An error naming the token; a redirect to `/login` |
...
| 13 | Invitee | `/accept-invitation/:token` | Submit with a blank name, then a 5-character password | Both refused with **400** and a message naming what is missing: "Name and password are required" (`:53-56`), and for the password a sentence that **begins** "Password must be at least 8 characters long" (`:58-60`). Do not assert the whole string: once the configured policy is enforced the message lists every unmet rule in one sentence, so a 5-character lowercase password reads "Password must be at least 8 characters long, contain an uppercase letter, contain a number". Assert the 400 and that the sentence names the length; see §6.6's box | A generic "Request failed" with nothing named — this repository has a standing lesson about exactly that |
...
> **The password you type in rows 3, 4 and 5 must satisfy the configured policy.**
>
> `/auth/signup` validates the password **before** it looks up the email domain
> (`AuthEndpoints.cs`: policy check, then `Companies.FirstOrDefault(c => c.EmailDomain == domain)`
> and the 404). So a password the policy refuses earns a **400 and never reaches the 404 branch
> row 3 exists to test** — and row 3's "failure looks like" column would have you file a
> tenant-provisioning defect (#280) for something that is only a weak password.
>
> The policy is whatever `GET /admin/system/settings` returns for the tenant. **With no settings
> row the entity defaults apply: minimum 8, an uppercase letter, a lowercase letter and a
> number.** `Uat-2026!` satisfies those and every stricter combination the admin screen can
> express, so use it unless the client has set a policy that refuses it — in which case read the
> policy off the admin screen first and pick accordingly. Do **not** use `test1234!`: it has no
> uppercase letter and the defaults refuse it.
>
> Record a 400 here as `blocked`, not `fail`, and say which rule the message named.
