...
| P11 | **Google OAuth origins are WRONG for `climate.timsint.com`** — no longer merely unconfirmed. | [MEASURED 2026-09-04 in the Google Cloud console, via browser: the **only** OAuth client in the `nodelabz` project is "Web client 2" (`762494443119-8u4u…`), whose JS origins are `https://lzhfnjfsdwdywwnlqgqq.supabase.co` and `http://localhost:3010`, and whose single redirect URI is `https://lzhfnjfsdwdywwnlqgqq.supabase.co/auth/v1/callback`. That is a **different** Supabase project (this platform's ref is `uleeeziiceduvmiftgby`) and a **Supabase Auth** callback, which this application does not use — it has its own `User.PasswordHash` and issues its own JWTs. `https://climate.timsint.com/auth/loading` appears nowhere. Not ruled out: a climate-specific GCP project that was not enumerated.] |
...
| B5 | Confirm the Google OAuth client's authorized JavaScript origins / redirect URIs include the final domain (the web flow redirects back to `<origin>/auth/loading`) | **UNCONFIRMED.** [VERIFIED 2026-09-02 (repo half only): `web/src/auth/googleOAuth.ts:68` — `export const GOOGLE_REDIRECT_PATH = '/auth/loading'`, used at line 130 as `${origin}${GOOGLE_REDIRECT_PATH}`. So the value that must be allowlisted is exactly `https://climate.timsint.com/auth/loading`.] [SUPERSEDED 2026-09-04 — verified from here after all, with a browser. **The value is not allowlisted.** The `nodelabz` project holds exactly one OAuth client and it belongs to a different application: origins `https://lzhfnjfsdwdywwnlqgqq.supabase.co` + `http://localhost:3010`, redirect `…supabase.co/auth/v1/callback`, last used 2026-08-03. So either this platform's client lives in a GCP project not yet enumerated, or Google sign-in was never configured for it. **If `VITE_GOOGLE_CLIENT_ID` on Vercel points at that client, the round trip fails with `redirect_uri_mismatch`** — the second branch `uat-script.md` §8.7 anticipated, which makes UAT §3.4 `blocked`. Next check, and it is cheap: read `VITE_GOOGLE_CLIENT_ID` in the Vercel project. If it is unset, no Google button renders and this is a non-event; if it is set to `762494443119-…`, this is a live cutover blocker.] |
