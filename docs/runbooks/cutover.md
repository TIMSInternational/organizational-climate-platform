...
| P4 | ~~**The API has no custom domain.**~~ **MET 2026-09-14** — `api.climate.timsint.com` is `active` with both ACM validation records `SUCCESS`, and serves the same `/version` commit as the App Runner hostname. The web bundle still calls the old host until `VITE_API_BASE_URL` is set and Vercel rebuilds (`docs/runbooks/api-custom-domain.md` step 8). Was: **The API has no custom domain.** #160 open. Everything external addresses it by its generated App Runner hostname. | `aws apprunner describe-custom-domains …/climate-project-api-prod` → `"CustomDomains": []`, `"DNSTarget": "bhgrdkd4gt.us-east-1.awsapprunner.com"`. |
...
| P11 | ~~**Google OAuth origins are unconfirmed for `climate.timsint.com`.**~~ **RESOLVED 2026-09-05 — not a blocker. Google sign-in is not configured anywhere in this deployment, and nothing in the shipped build ever sends a user to Google.** The origins really are not allowlisted, but that cannot break a cutover, because no client id is set on either side. It becomes a decision instead: *is Google sign-in in scope for go-live?* If yes, three things must be done together (see B5). If no, close B5/P11 as **not applicable** and mark UAT §3.4 `n/a`. | Measured in three places, and all three agree. **(1) Browser (Vercel console):** the `climate` project has exactly **two** environment variables, both `VITE_API_BASE_URL` (Preview + Production); `VITE_GOOGLE_CLIENT_ID` is **unset**, and the *Shared* tab reads "No shared variables linked". **(2) Browser (the live site):** `https://climate.timsint.com/login` renders **no "Continue with Google" button and no `auth.or` divider** — email, password, Sign In, nothing else. `LoginPage.tsx:164` gates the whole block on `googleClientId()`, so this is the deployed build confirming (1) independently. **(3) AWS (read-only `apprunner describe-service`):** the production service carries **15 runtime environment variables and 5 secrets, and no key on either list matches `google` case-insensitively** — nor does the string appear in `infra/` or `.github/workflows/`. `appsettings.json:10` ships `"GoogleClientId": ""`. So `POST /auth/google` would throw `InvalidOperationException("Missing GoogleClientId configuration.")` from `GoogleTokenVerifier`'s constructor if it were ever called; it is `AddScoped` (`Program.cs:335`), so the failure is per-request, not at startup, which is why the API boots fine. `GoogleAuth:Required` is `false` (`appsettings.json:11-13`) — the flag that exists precisely to allow this state (`StartupOptions.cs:79-91`). **Still true and still unenumerated:** the one OAuth client in the `nodelabz` project ("Web client 2", `762494443119-8u4u…`) belongs to a different application, and a climate-specific GCP project was not ruled out — but with no client id configured on either side, that no longer gates anything. |
...
| B5 | Confirm the Google OAuth client's authorized JavaScript origins / redirect URIs include the final domain (the web flow redirects back to `<origin>/auth/loading`) | **NOT APPLICABLE AS WRITTEN — there is no OAuth client to confirm.** [VERIFIED 2026-09-02 (repo half only): `web/src/auth/googleOAuth.ts:68` — `export const GOOGLE_REDIRECT_PATH = '/auth/loading'`, used at line 130 as `${origin}${GOOGLE_REDIRECT_PATH}`. So the value that would have to be allowlisted is exactly `https://climate.timsint.com/auth/loading`.] [MEASURED 2026-09-04, browser: that value is allowlisted nowhere. The `nodelabz` project holds exactly one OAuth client and it is a different application's — origins `https://lzhfnjfsdwdywwnlqgqq.supabase.co` + `http://localhost:3010`, redirect `…supabase.co/auth/v1/callback`, last used 2026-08-03. Not this platform's Supabase ref (`uleeeziiceduvmiftgby`), and a Supabase Auth callback this application does not use.] [RESOLVED 2026-09-05 — see P11 for the three measurements. **`VITE_GOOGLE_CLIENT_ID` is unset on Vercel and `GoogleClientId` is absent from the production App Runner service**, so no user can reach Google and no `redirect_uri_mismatch` can occur. **This step gates nothing on the day.**] **If Google sign-in is wanted at go-live, it is three changes, not one, and they must ship together:** (a) create an OAuth client in a GCP project for *this* platform and allowlist origin `https://climate.timsint.com` + redirect `https://climate.timsint.com/auth/loading`; (b) set `VITE_GOOGLE_CLIENT_ID` in the Vercel project `climate` — **build-time, so it needs a redeploy, not just a save**; (c) set `GoogleClientId` on the App Runner service to the *same* value, or `POST /auth/google` 500s on every attempt. Doing (b) alone is the worst outcome: it renders a button that always fails. |
