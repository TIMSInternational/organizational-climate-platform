...
| ~~`deploy-drift.yml` runs daily and has passed every day since 2026-08-16~~ → **it runs daily and has FAILED on six of its last eight runs.** [CORRECTED 2026-09-03. Measured `gh run list --workflow=deploy-drift.yml --limit 8 --json createdAt,conclusion`: `2026-09-03T16:56:58Z` **success**, `2026-09-02T17:01:46Z` **success**, `2026-09-01T17:11:24Z` failure, `2026-08-31T19:24:45Z` failure, `2026-08-30T17:13:28Z` failure, `2026-08-29T17:00:48Z` failure, `2026-08-28T22:54:37Z` failure, `2026-08-27T22:50:41Z` failure. The failing job is "Compare live /version against main" ("Production is N commits behind main (threshold 20)"); the 2026-09-02 deploy of `b371a9d`, and today's of `e0896f9`, are what cleared it. **Why this row mattered more than the others:** telling an operator the check "has passed every day" invites them to read a red run as new when it is a week old, or to skip the listing entirely. This table is headed "Verified today" with **no date** — that is what let this row and its neighbours age silently; the neighbouring `fc53936` / "23 commits behind" rows are of the same undated 2026-08-19 vintage and production now serves `e0896f9` (`GET /version`, 2026-09-03).] | `gh run list --workflow=deploy-drift.yml` |
| Today's drift run (13:52 UTC) passed at ≤20 commits behind | `gh run view 32735215787 --log` |
| ~~**Mail is not configured in production**~~ → **mail IS configured in production, and has been since 2026-08-26.** [CORRECTED 2026-09-03.] | [WAS: "`infra/aws/climate-project-api-prod-service.yml` contains zero `Email__` variables".] `grep -n "Email__" infra/aws/climate-project-api-prod-service.yml` → **eleven** hits: eight plain variables at lines 265-293 (`Email__Provider=smtp`, `Email__SmtpHost=email-smtp.us-east-1.amazonaws.com`, `Email__SmtpPort`, `Email__SmtpUseStartTls`, `Email__FromAddress`, `Email__FromName`, `Email__AppBaseUrl=https://climate.timsint.com`, `Email__SesConfigurationSet`) plus `Email__SmtpUsername`/`Email__SmtpPassword` as `RuntimeEnvironmentSecrets` at 302-305. The **live** service carries them: `aws --profile claude apprunner describe-service --service-arn arn:aws:apprunner:us-east-1:747814092517:service/climate-project-api-prod/126c3f282524450896385975cb3bcba9 --query "…RuntimeEnvironmentVariables"` → `"Email__Provider": "smtp"`, `"Email__SmtpHost": "email-smtp.us-east-1.amazonaws.com"`, `"Email__AppBaseUrl": "https://climate.timsint.com"`. Secret ARNs are on the `production` environment (`EMAIL_SMTP_USERNAME_SECRET_ARN`, `EMAIL_SMTP_PASSWORD_SECRET_ARN`, both `2026-08-26T17:47Z`, `gh variable list --env production`). **What is still NOT measured: a real message received in a real inbox.** Configuration is an inference about a template; delivery is a mailbox. |
...
| `services/tracking-api` has two background workers and ~~no deploy path~~ → **a deploy path that has never succeeded.** [CORRECTED 2026-09-05. The evidence cited here stopped being true on 2026-08-26.] | `ClimateTracking.Workers/{CacheSyncWorker,DailySemaforoWorker}.cs` — unchanged. [WAS: "`grep -rn tracking-api .github/workflows/` returns nothing".] That grep now returns **`ci.yml` and `deploy-tracking-prod.yml`**; `services/tracking-api/Dockerfile` and `infra/aws/climate-tracking-api-{bootstrap,prod-service}.yml` also exist — all four added by `8463027a` (#396, 2026-08-26). **The operationally relevant fact is unchanged and is why this row still belongs here:** the workers run in **no** environment. `deploy-tracking-prod.yml` has exactly one lifetime run (`2026-08-27T21:21:19Z`, **failure**) which stopped at its third step, `Verify deploy configuration is present`, so the image was never built. A missing artifact would be an engineering task; this is a provisioning one. Same correction as `rollback.md` H7 (made 2026-09-01) and `staging-provisioning.md` Step 9 — this file and that one both kept the stale claim for four more days. |
