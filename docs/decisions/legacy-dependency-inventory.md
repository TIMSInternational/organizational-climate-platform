...
| DNS records and TTLs for `timsint.com` | Namecheap (`dns1/dns2.registrar-servers.com`). [AMENDED 2026-09-14: the **`climate.timsint.com` subtree is no longer among them** — it is delegated to Route 53 public zone `Z058256939JG0YA4Y6FRY` in the prod account, so `aws route53 list-resource-record-sets --hosted-zone-id Z058256939JG0YA4Y6FRY` now *does* read those records. The apex `timsint.com` and its TTLs remain Namecheap-only. Why: `docs/runbooks/api-custom-domain.md`.] | Namecheap console → `cutover.md` step B1; the `climate` subtree via `aws route53` |
