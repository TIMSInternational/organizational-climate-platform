...
| **And that off-switch is inert on a database with no `system_settings` row.** The method opens `var settings = await db.SystemSettings.FirstOrDefaultAsync(...)` and returns `null` — i.e. *no gate* — when `settings is null`. A fresh or restored database that has never had the settings row written therefore has neither kill switch, and signup is reachable with no off-switch of any kind. This is not hypothetical for a cutover: the row is created by an admin action, not by a migration | `AuthEndpoints.cs:422-426` |
