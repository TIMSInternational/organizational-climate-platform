...
/// <summary>
/// Procomer acceptance criterion 7: the plans can be exported as a workbook with the same
/// structure as the client's own template — their sheet name, their thirteen columns, in their
/// order. See <see cref="TrackingSheetExport"/> for the column contract, the transforms, why the
/// workbook format is what the criterion needs, and why this direction is the only direction.
/// </summary>
public static class TrackingSheetExportEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var fileName = string.Create(
            ...
            $"seguimiento-planes-accion-{DateTime.UtcNow:yyyy-MM-dd}{TrackingSheetExport.FileExtension}");

        // No charset parameter: a workbook is a zip container, not text, and appending one is
        // how a browser is talked into saving this as something Excel then refuses to open.
        return Results.File(
            TrackingSheetExport.Build(planes, lookups), TrackingSheetExport.ContentType, fileName);
    }
