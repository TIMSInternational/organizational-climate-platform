...
describe('the team against the organisation', () => {
  ...
  /**
   * The LeaderDashboard artboard's rule: the side of the target the PRINTED reading falls on.
   * Reconocimiento 3,5 is "bajo la meta 3,7" with Crear plan; Carga de trabajo 3,67 prints 3,7
   * and is "en la meta"; 3,75 prints 3,8 and is over it.
   */
  it('stands each reading on its side of the target at the printed decimal, as the leader artboard draws it', () => {
    expect(dimensionStanding({ key: 'a', team: 3.17, organization: null }, 3.7)).toBe('below')
    expect(dimensionStanding({ key: 'a', team: 3.5, organization: null }, 3.7)).toBe('below')
    expect(dimensionStanding({ key: 'a', team: 3.64, organization: null }, 3.7)).toBe('below')
    expect(dimensionStanding({ key: 'a', team: 3.66, organization: null }, 3.7)).toBe('on')
    ...
    expect(dimensionStanding({ key: 'a', team: 3.75, organization: null }, 3.7)).toBe('above')
