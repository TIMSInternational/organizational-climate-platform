...
import { climateDetail, surveyDimensionScore, type ClimateDimension } from '../surveyResultsMap'
 ...
 * Control reads, so the two screens cannot classify one dimension two ways. Nothing
 * on the wire carries a target yet (see `compose.ts`); `surveyResultsMap.ts` colours
 * its own map against the survey's mean because it predates the target, and this
 * page deliberately does not: "bajo la meta" and "bajo la media" are different
 * claims, and the artboard makes the first one.
 *
 ...
 * is not symmetric: a reading at or above it is *over* the goal (blue), a reading
 * within `ON_TARGET_TOLERANCE` under it is *at* the goal (grey — the width of two
 ...
 * thresholds are what the approved artboard tints — 3,5 to 3,7 grey, 3,8 to 4,0 light
 * blue, 4,2 and up dark blue, 3,4 down to 2,8 light red, 2,6 and under dark red — and
 * the whole test of this function is that table.
 */
...
export interface ResultsCellDetail {
  ...
  others: { id: string; name: string; isProtected: boolean; score: number | null; band: TargetBand | null }[]
  plan: ResultsPlanRef | null | undefined
}
  ...
  return {
    ...
    others: rows
      ...
      .map((row) => {
        ...
        return { id: row.id, name: row.name, isProtected: row.isProtected, score, band: score === null ? null : targetBand(score) }
      }),
