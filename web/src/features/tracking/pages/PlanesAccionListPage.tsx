...
import { SEMAFORO_ORDER, semaforoPresentation, tallySemaforo } from '../semaforo'
...
export default function PlanesAccionListPage() {
  ...
  // `tallySemaforo`, not a loop written out here — see `semaforo.ts`. `total` is
  // the row count, so a state this build does not recognise shows up as a
  // disclosed shortfall on the strip instead of silently vanishing from the counts.
  const counts = useMemo<SemaforoCounts>(() => tallySemaforo(plans), [plans])
