 ...
 * round caps and joins (`MicroclimatesList.dc.html`, `MicroclimateLive.dc.html`:
 * `<svg style="width: 16px; height: 16px; stroke: …; fill: none; stroke-width: 2; …"
 * viewBox="0 0 16 16">`). Lucide draws in a 24-unit box stroked 2, so at 16px its line is
 * 1.33px against the canvas's 2px, and three of its nearest neighbours are different
 * pictures: a two-loop chain where the canvas has one diagonal link, and an outlined page
 * and an outlined pair of sheets where the canvas's render (the PNG Federico approved) has
 * both bodies solid. The paths are copied verbatim from the artboards' sources; the two
 * solid bodies are filled because that is what the approved render shows, although the
 * source writes `fill: none` on the element.
 *
...
/** "Resultados": a page with its corner folded, the body solid as the render draws it. */
export function CanvasFileIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <CanvasGlyph {...props}>
      <path d="M4 2h5l3 3v9H4z" fill="currentColor" />
      <path d="M9 2v3h3" />
    </CanvasGlyph>
  )
}

/** "Copiar": the front sheet solid, the one behind it an open corner. */
...
/** "Protegido hasta 5 respuestas", and the hatch's padlock: outlined, as the render draws it. */
