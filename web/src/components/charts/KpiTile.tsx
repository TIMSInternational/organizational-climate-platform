...
export interface KpiTileProps {
   ...
   * "respuestas · 100 % completadas". Already translated. The artboards put the
   * unit beside the number and keep `sub` for the one coloured sentence under it;
   * a caller that has only a sentence leaves this off.
   */
...
export default function KpiTile({
  ...
  return (
    <div
      ...
      data-slot="kpi-tile"
      className={cn(
        // The canvas's `.card`: the panel surface with the default hairline, 14px
        ...
        // and SurveyResults artboards' white cards; both screens draw this one
        // primitive, so the surface is corrected here once.
        'rounded-lg border border-line-default bg-surface-panel px-4 py-3.5',
        className,
      )}
    >
      ...
      <div className="mt-px flex items-center gap-1 text-sm text-fg-secondary">
        {hasChange && (
          <span
            className={cn(
              'font-semibold',
