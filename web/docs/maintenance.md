...
| `/maintenance.html` is reachable on a Vercel deployment despite the catch-all rewrite | **Expected from Vercel's reference** (filesystem before rewrites). **Not yet observed** on this project — the next production or preview deployment that carries this file proves it with one `curl -I https://climate.timsint.com/maintenance.html` (expect `200`, `text/html`). |
