...
public class ActionPlanTemplateItemsTests(PostgresContainerFixture postgres)
{
    ...
    public async Task Template_kpi_and_objective_rows_round_trip()
    {
        ...
        {
            Id = Guid.NewGuid(), TemplateId = template.Id, NameEn = "Response rate",
            TargetValue = 85m, UnitEn = "%", MeasurementFrequency = "monthly",
        };
        var objective = new ActionPlanTemplateObjective
        {
            Id = Guid.NewGuid(), TemplateId = template.Id,
            DescriptionEn = "Improve team cohesion", SuccessCriteriaEn = "Two team events run",
        };
        ...
        Assert.Equal("Two team events run", loadedObjective.SuccessCriteriaEn);
    }
    ...
    public async Task Deleting_a_template_cascades_delete_of_its_kpi_and_objective_rows()
    {
        ...
        {
            Id = kpiId, TemplateId = template.Id, NameEn = "K", TargetValue = 1m, UnitEn = "count", MeasurementFrequency = "weekly",
        });
        db.ActionPlanTemplateObjectives.Add(new ActionPlanTemplateObjective
        {
            Id = objectiveId, TemplateId = template.Id, DescriptionEn = "d", SuccessCriteriaEn = "s",
        });
        await db.SaveChangesAsync();
